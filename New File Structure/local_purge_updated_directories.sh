...
local_path=/var/panda/CyverseData/JmscslgroupData/PandaData/$VIN
...
for file in $local_files; do
  ...
  # Add files to log for the first time
    ...
    if [[ $file == *.csv ]]; then 
      year_file=${name:0:4} #FIX
      month_file=${name:5:2} #FIX
      day_file=${name:8:2} #FIX
      time_file=${name:11:8} #FIX
      ...
      local_size=$(du --bytes /var/panda/CyverseData/JmscslgroupData/PandaData/$VIN/$date_directory/$name | awk '{print $1}')
